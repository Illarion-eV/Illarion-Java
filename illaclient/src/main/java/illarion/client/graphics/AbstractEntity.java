...
public abstract class AbstractEntity<T extends AbstractEntityTemplate>
    ...
    /**
     * The base color is the color the image of the sprite is always colored with, this color is applied no matter what
     * the localLight is set to.
     */
    @Nullable
    ...
    /**
     * Once this value is set {@code true} the entity can be assumed to be removed. It must not be added to the display
     * again once this was done.
     */
    private boolean removedEntity;
    private boolean affectedByFC;
    @Nullable
     ...
     * entity will move closer to the alpha target. To set the alpha value without a fading animation use {@link
     * #setAlpha(int)}.
     *
     * @param newAlpha the target of the alpha fading
     */
    @Override
    public final void setAlphaTarget(int newAlpha) {
        if (removedEntity) {
            LOGGER.warn("Changing the alpha animation target of a entity is not allowed.");
            return;
        }
        alphaTarget = FastMath.clamp(newAlpha, 0, Color.MAX_INT_VALUE);
    }
    ...
    /**
     * Get the Z Order of this entity that marks the position in the display list and selects this way, how other images
     * overlay this entity.
     *
    ...
    /**
     * Draw this entity to the screen. This also performs a few basic animations such as fading in and out, based on the
    ...
    public void render(@Nonnull Graphics graphics) {
        if (performRendering()) {
            ...
            if ((highlight > 0) && (highlightEffect != null)) {
                ...
                renderSprite(graphics, renderLocX, renderLocY, renderLight, highlightEffect);
            } else {
                renderSprite(graphics, renderLocX, renderLocY, renderLight);
            }
        }
    }
    ...
    public void update(@Nonnull GameContainer container, int delta) {
        ...
        if (fadingCorridorEffect) {
            FadingCorridor fc = FadingCorridor.getInstance();
            affectedByFC = fc.isInCorridor(displayRect.getX(), displayRect.getY(), dC.getLayer(), widthNoShadow,
    ...
    /**
     * Calling this function marks the entity to be removed from the client for good. Once this function was called its
     * not allowed to do anything anymore with this entity.
     */
    ...
    /**
     * This function checks if the entity is made transparent due the color its drawn with.
     *
     * @return {@code true} in case the graphic is turned transparent due its color
     */
    ...
    /**
     * Update the alpha value. This function causes the alpha value to approach the target alpha value and notifies in
     * case its needed all listeners.
     *
