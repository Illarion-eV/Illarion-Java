 ...
 */
public final class GameMapHandler implements
    EventTopicSubscriber<DragOnMapEvent> {
    /**
     * This class is used as end operation to the dragging that started on the
     * map. It takes care that the elements used to perform the dragging and
     * cleaned up properly.
     * 
     * @author Martin Karing &lt;nitram@illarion.org&gt;
     */
    private static final class GameMapDragEndOperation implements Runnable {
         ...
         * @param returnToElement the element the dragged element needs to
         *            return to
         */
        public GameMapDragEndOperation(final Element draggedElement,
            final Element returnToElement) {
            drag = draggedElement;
    ...
    /**
     * The panel that is located on top of the game map. So this is the lowest
     * located panel and the intended parent for the original location of the
     * dragged object.
     */
    ...
    /**
     * Default constructor that takes care to initialize the variables required
    ...
    public void dropOnMap(final String topic, final DroppableDroppedEvent data) {
        ...
        final int dropSpotX =
            droppedElement.getX() + (droppedElement.getWidth() / 2);
        final int dropSpotY =
            droppedElement.getY() + (droppedElement.getHeight() / 2);
    ...
    public void handleDragOnMap(final int oldx, final int oldy,
        ...
        if (activeScreen != null && activeNifty != null) {
            ...
            draggedGraphic.setConstraintWidth(new SizeValue(Integer
                .toString(width) + "px"));
            draggedGraphic.setConstraintHeight(new SizeValue(Integer
                .toString(height) + "px"));
            draggedGraphic.setConstraintX(new SizeValue(Integer.toString(oldx
                - width / 2)
                + "px"));
            draggedGraphic.setConstraintY(new SizeValue(Integer.toString(oldy
                - height / 2)
                + "px"));
            ...
            final ImageRenderer imgRender =
                draggedImage.getRenderer(ImageRenderer.class);
            imgRender.setImage(new NiftyImage(activeNifty.getRenderEngine(),
                new EntitySlickRenderImage(movedItem)));
