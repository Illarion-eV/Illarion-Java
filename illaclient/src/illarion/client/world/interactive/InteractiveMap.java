...
public final class InteractiveMap {
    ...
    public InteractiveMapTile getInteractiveTileOnDisplayLoc(
        final int displayX, final int displayY) {
        return getInteractiveTile(getTileOnDisplayLoc(displayX, displayY));
    }

    public InteractiveMapTile getInteractiveTileOnMapLoc(final int locX,
        final int locY, final int locZ) {
    ...
    public InteractiveMapTile getInteractiveTileOnScreenLoc(final int screenX,
        final int screenY) {
    ...
    public MapTile getTileOnDisplayLoc(final int displayX, final int displayY) {
        ...
        for (int i = 4; i >= 0; --i) {
            ...
            foundTile =
                parentMap.getMapAt(lowX - levelOffset, lowY + levelOffset,
                    base + i);
    ...
    public MapTile getTileOnMapLoc(final int locX, final int locY,
        final int locZ) {
    ...
    public MapTile getTileOnScreenLoc(final int screenX, final int screenY) {
        ...
        return getTileOnDisplayLoc(displayManager.getWorldX(screenX),
            displayManager.getWorldY(screenY));
    }
}
