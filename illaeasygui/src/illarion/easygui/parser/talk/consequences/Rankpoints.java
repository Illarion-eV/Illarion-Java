...
import illarion.easygui.parsed.talk.consequences.ConsequenceRankpoints;
        ...
        if (stringMatcher.find()) {
            ...
            for (final Towns testTown : Towns.values()) {
                if (testTown.validForRankpoints()
                    && groupName.equalsIgnoreCase(testTown.name())) {
                    town = testTown;
                    break;
                }
            }
            ...
            if (town == null) {
                reportError(String.format(Lang.getMsg(getClass(), "group"),
                    groupName, stringMatcher.group(0)));
                return extract();
            }
            ...
            final ConsequenceRankpoints rankpointCons =
                ConsequenceRankpoints.getInstance();
            rankpointCons.setData(town, operator, targetValue);
            return rankpointCons;
        }
