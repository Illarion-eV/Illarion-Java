...
import illarion.easygui.ScriptWriter;
...
final class Utils {
    ...
    /**
     * Save a script in the editor as easyGUI script. This saves the current
    ...
    /**
     * Display a file selection dialog to allow the player to select a easyGUI
    ...
    protected static void selectAndOpenScript() {
        ...
        fileDiag.setFileFilter(new FileFilter() {
            ...
            @Override
            public String getDescription() {
                return Lang.getMsg(Utils.class, "easyGUIScriptsFileType"); //$NON-NLS-1$
            }
        });
        fileDiag.setCurrentDirectory(new File(Config.getInstance()
            .getEasyNpcFolder()));
     ...
     * easyGUI script. Once selected the function saves the current state of the
     * script.
     * 
     * @param editor the editor that supplies the script text
     */
    ...
    private static void saveEasyNPCImpl(final String script,
        ...
        try {
            ...
            LOGGER.error("Writing the easyGUI Script failed.", e); //$NON-NLS-1$
        }
    }
}
