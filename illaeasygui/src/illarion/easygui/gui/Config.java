...
public final class Config implements ConfigChangeListener {
    ...
    /**
     * The property key value for the easyGUI script folder.
     */
    @SuppressWarnings("nls")
    private static final String easyGUIFolder = "easyGUIFolder";
    ...
    public ConfigDialog createDialog() {
        ...
        page = new ConfigDialog.Page("illarion.easygui.gui.config.generalTab");
        page.addEntry(new ConfigDialog.Entry(
            "illarion.easygui.gui.config.easyGUIFolderLabel",
            new DirectoryEntry(easyGUIFolder, null)));
        page.addEntry(new ConfigDialog.Entry(
            "illarion.easygui.gui.config.luaFolderLabel", new DirectoryEntry(
                luaNpcFolder, null)));
        page.addEntry(new ConfigDialog.Entry(
            "illarion.easygui.gui.config.maxUndoLabel", new NumberEntry(
                undoCountKey, 0, 10000)));
        page.addEntry(new ConfigDialog.Entry(
            "illarion.easygui.gui.config.errorReport", new SelectEntry(
                illarion.common.bug.CrashReporter.CFG_KEY,
                SelectEntry.STORE_INDEX, Lang
                    .getMsg("illarion.easygui.gui.config.errorAsk"), Lang
                    .getMsg("illarion.easygui.gui.config.errorAlways"), Lang
                    .getMsg("illarion.easygui.gui.config.errorNever"))));
        dialog.addPage(page);

        page =
            new ConfigDialog.Page("illarion.easygui.gui.config.lookAndFeelTab");
        page.addEntry(new ConfigDialog.Entry(
            "illarion.easygui.gui.config.useWindowDecoLabel", new CheckEntry(
                useWindowDeco)));
        ...
        page.addEntry(new ConfigDialog.Entry(
            "illarion.easygui.gui.config.usedThemeLabel", new SelectEntry(
        ...
        page.addEntry(new ConfigDialog.Entry(
            "illarion.easygui.gui.config.useSyntaxLabel", new CheckEntry(
                useSyntaxHighlight)));
        dialog.addPage(page);
    ...
    /**
     * Get the folder where to store the easyGUI scripts.
     * 
     * @return the folder to store the easyGUI scripts
     */
    public String getEasyNpcFolder() {
        return cfg.getFile(easyGUIFolder).toString();
    }
    ...
    public void init() {
        ...
        configFile = new File(folder, "easyguieditor.xcfgz");
        cfg = new ConfigSystem(configFile);

        cfg.setDefault(lastFilesKey, "");
        cfg.setDefault(easyGUIFolder,
    ...
    /**
     * Set the folder where to store the easyGUI scripts.
     * 
     * @param newFolder the folder where to store the easyGUI scripts
     */
    public void setEasyNpcFolder(final String newFolder) {
        cfg.set(easyGUIFolder, new File(newFolder));
    }
