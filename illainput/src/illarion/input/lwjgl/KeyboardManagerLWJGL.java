...
public final class KeyboardManagerLWJGL implements KeyboardManager {
     ...
     * states.
     */
    private List<KeyboardEventReceiver> receivers = new FastTable<KeyboardEventReceiver>();
    ...
    public void poll() {
        ...
        while (Keyboard.next()) {
            ...
            KeyboardEventReceiverPrimitive receiverPriminitve;
            KeyboardEventReceiverComplex receiverComplex;
            for (KeyboardEventReceiver receiver : receivers) {
                if (receiver instanceof KeyboardEventReceiverPrimitive) {
                    receiverPriminitve = (KeyboardEventReceiverPrimitive) receiver;
                    receiverPriminitve.handleKeyboardEvent(keyCode, eventChar, pressed);
                } else if (receiver instanceof KeyboardEventReceiverComplex) {
                    receiverComplex = (KeyboardEventReceiverComplex) receiver;
                    
                    if (pressed && !repeated) {
                        event.setEventData(keyCode, KeyboardEvent.EVENT_KEY_DOWN,
                            false, eventChar);
                        receiverComplex.handleKeyboardEvent(event);
    ...
    @Override
    public void unregisterEventHandler(KeyboardEventReceiver event) {
