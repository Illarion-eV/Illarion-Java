...
import illarion.graphics.Sprite.HAlign;
import illarion.graphics.Sprite.VAlign;
 ...
 * @since 2.00
 */
public final class SpriteLWJGL implements Sprite {
    /**
     ...
     */
    private final TextureLWJGL[] textures;
    ...
    public SpriteLWJGL(final int frames) {
        ...
        textures = new TextureLWJGL[frames];
     ...
     */
    public SpriteLWJGL(final SpriteLWJGL org) {
    ...
    public void addTexture(final Texture newTexture) {
        ...
        if ((textures.length - unsetTextures) > 0) {
            final TextureLWJGL firstTex = textures[0];
    ...
    public final int getHeight() {
        final TextureLWJGL texture = textures[0];
        if (texture == null) {
            return -1;
        }
     ...
     */
    @Override
    public final TextureLWJGL getTexture(final int index) {
    ...
    public final void remove() {
        for (final TextureLWJGL texture : textures) {
            texture.remove();
            texture.cleanup();
            unsetTextures++;
        }
    ...
    public void drawNasty(int x, int y, int w, int h, int centerX,
        ...
        final TextureLWJGL tex = getTexture(0);
        
        DriverSettingsLWJGL.getInstance().enableMode(DriverSettingsLWJGL.Modes.DRAWTEXTURE, tex.getParent());
        ...
        GL11.glTexCoord2f(tex.getRelX1(), tex.getRelY1()); GL11.glVertex2i(x,     y);
        GL11.glTexCoord2f(tex.getRelX2(), tex.getRelY1()); GL11.glVertex2i(x + w, y);
        GL11.glTexCoord2f(tex.getRelX2(), tex.getRelY2()); GL11.glVertex2i(x + w, y + h);
        GL11.glTexCoord2f(tex.getRelX1(), tex.getRelY2()); GL11.glVertex2i(x,     y + h);
        GL11.glEnd();
