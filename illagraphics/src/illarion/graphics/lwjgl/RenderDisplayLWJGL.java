...
public final class RenderDisplayLWJGL implements RenderDisplay { // NO_UCD
    ...
    public int getObjects() {
        return SpriteLWJGL.getDrawnObjects();
    }
...
//        GL11.glScissor(x, y, width, height);
    ...
    public void setDisplayMode(final GraphicResolution resolution) {
        final List<DisplayMode> modes = getDisplayModes();

        for (DisplayMode mode : modes) {
            if (mode.getWidth() == resolution.getWidth()
                && mode.getHeight() == resolution.getHeight()
                && mode.getBitsPerPixel() == resolution.getBPP()) {
                setDisplayMode(mode);
                break;
            }
        }
    ...
    private void setDisplayMode(final DisplayMode mode) {
        resWidth = mode.getWidth();
        resHeight = mode.getHeight();
        updateSyncFreq();
        try {
    ...
    protected void setupOpenGL() {
        ...
        GLU.gluOrtho2D(0, resWidth, resHeight, 0);
