...
public final class VorbisSPI implements LibraryResource {
     ...
     */
    private static final VorbisSPI INSTANCE = new VorbisSPI();
    ...
    public Collection<File> getClassPath() {
        if (classpath == null) {
            ...
            cp.add(new File(dataDir, "vorbisspi.jar")); //$NON-NLS-1$
    ...
    public String getName() {
        return Lang.getMsg(VorbisSPI.class.getName());
    }
    ...
    public Collection<URL> getRequiredRessources() {
        if (resources == null) {
            ...
            try {
                res.add(new URL(ONLINE_PATH + "vorbisspi" + RESSOURCE_FILE_EXT)); //$NON-NLS-1$
    ...
    public Collection<String> getVMArguments() {
        if (vmArguments == null) {
            ...
            vmArgs.add("-Dillarion.components.avaiable.jorbis=true"); //$NON-NLS-1$
            vmArgs.add("-Dillarion.components.avaiable.jogg=true"); //$NON-NLS-1$
            vmArgs.add("-Dillarion.components.avaiable.tritonus=true"); //$NON-NLS-1$
            vmArgs.add("-Dillarion.components.avaiable.vorbisspi=true"); //$NON-NLS-1$
