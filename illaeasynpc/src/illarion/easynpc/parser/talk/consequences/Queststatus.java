...
/**
 * This is the quest status consequence. Its able to parse a quest status consequence out of the consequence
 * collection string.
 *
...
public final class Queststatus extends ConsequenceParser {
    ...
    private static final Pattern STRING_FIND = Pattern.compile("\\s*queststatus\\s*\\(\\s*(\\d+)\\s*\\)\\s*" +
    ...
    public TalkConsequence extract() {
        ...
        if (stringMatcher.find()) {
            ...
            return new ConsequenceQueststatus(questid, operator, targetValue);
        }
