...
/**
 * This is a rank condition. Its able to parse a rank value out of the NPC condition line.
 *
 * @author vilarion &lt;vilarion@illarion.org&gt;
 */
public final class Rank extends ConditionParser {
    /**
     * This pattern is used to find the rank operation in the condition
     * properly.
     */
    @SuppressWarnings("nls")
    private static final Pattern RANK_FIND = Pattern.compile("\\s*rank\\s*([=<>]{1,2})\\s*"
    ...
    public TalkCondition extract() {
        ...
        final Matcher stringMatcher = RANK_FIND.matcher(getNewLine());
        if (stringMatcher.find()) {
            ...
            return new ConditionRank(operator, targetValue);
        }
    ...
    public void enlistHighlightedWords(@Nonnull final TokenMap map) {
        map.put("rank", Token.RESERVED_WORD);
    }
}
